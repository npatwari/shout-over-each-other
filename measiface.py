...
class MeasurementsInterface:
    ...
    # This is Neal's modified save iq w tx function.
    # It is a simultaneous transmission schedule, not a one-at-a-time TX schedule.
    # It tests every possible combination of nodes being used as transmitters.
    # That is, in one round each node can be either a TX or RX.
    # This schedule iteratively tests all possible ways to set the nodes
    # except for the case when all nodes are transmitters (bc we can't measure this).
    def cmd_saveiqwtx(self, cmd): # For sequential transmitters
        ...
        # Compute total measurements (This may be wrong now)
        ...
        print("Total measurmemets with TX = %d, one TX duration = %d" %(totalmeas, cmd['duration']))
        ...
        # Second, measure with each combination of TXs
        # How many combinations of transmitters are there?
        num_tx = len(txclients)
        count_tx_combos = 2**num_tx

        # Every integer i with num_TXs digits has a binary representation. 
        # Each bit is 1 if that node will TX in this round.
        # Each bit is 0 if that node will NOT TX in this round. 
        # Thus there's a 1-1 conversion between an int and which nodes tx in this round.
        # But skip the case when every single node is transmitting (if there is no node
        # to receive, have the TXes really transmitted?)
        for i in range(count_tx_combos-1):
            # convert integer i in (0, 2**(num_TXs) to a num_TXs-length T/F list of which tx is included in this round
            # tx_combo, combined_str = convert_int_to_tx_combo(i, txclients, num_tx)
            # Convert integer i to num_tx length list of binary values 
            tx_combo = [(x=="1") for x in list(bin(i)[2:].zfill(num_tx))]
            print("Nodes transmitting: " + str(tx_combo))

            # Write a string with each txclient string (that is in this combo) concatenated
            combined_str = '0'
            for j, tx_name in enumerate(txclients):
                if tx_combo[j]:
                    combined_str = combined_str + "+" + tx_name

            print("Transmitting from: %s" %combined_str)
            txgrp = measgrp.create_group(combined_str)

            # Keeping the code to have multiple possible tx gains 
            for tgain in np.arange(txgain['start'], txgain['end'], txgain['step']):
                ...
                # Create a regular python list of receivers, transmitters for this round
                rxlist = []
                for j, node_name in enumerate(txclients):
                    if tx_combo[j]:
                        txcmd.clients.append(node_name)
                    else:
                        rxlist.append(node_name)
                print("txcmd.clients: " + str(txcmd.clients))

                txcmd.start_time = time.time() + toff
                txcmd.uuid = random.getrandbits(31)
                temp_str = txcmd.SerializeToString()
                # print(temp_str) # Can print to screen as a debug
                self.pipe.send(temp_str)
                ...
                self._do_saveiq_set(rxgain, "tx gain: "+str(tgain) + ", ", cmd, rxlist, txgaingrp, cmd['rxrepeat'], rxwait)
            ...
            self.cmd_waitres({'client_list': txcmd.clients,'timeout': cmd['timeout']})
            print("Done")
